...
public class JavaMOPExtension extends AbstractEventSpy {
    ...
    private static final String profileFile = "profile-" + java.util.UUID.randomUUID() + ".jfr";

    private static final String ASYNC_PROFILER_STRING = "-agentpath:/home/Valg/valg-study/scripts/async-profiler-2.9-linux-x64/build/libasyncProfiler.so=start,interval=5ms,event=wall,file=" + profileFile;
    ...
    private void updateConfig(Xpp3Dom config) {
        ...
        if (argLine != null) {
            if (!jvmOptions.isEmpty()) {
                argLine.setValue(argLine.getValue() + " " + agentPath + jvmOptions + " " + ASYNC_PROFILER_STRING);
            } else {
                argLine.setValue(argLine.getValue() + " " + agentPath + " " + ASYNC_PROFILER_STRING);
            }
        } else {
            argLine = new Xpp3Dom("argLine");
            if (!jvmOptions.isEmpty()) {
                argLine.setValue(agentPath + jvmOptions + " " + ASYNC_PROFILER_STRING);
            } else {
                argLine.setValue(agentPath + " " + ASYNC_PROFILER_STRING);
            }
