    ...
    public static void main(String[] args) {
        ...
        if (args.length >= 3)  {
            if (!args[2].equalsIgnoreCase("naive") && !args[2].equalsIgnoreCase("parent") &&
                ...
                !args[2].equalsIgnoreCase("gc") && !args[2].equalsIgnoreCase("default")
            ) {
                System.out.println("classifier: naive, parent, hot, violation, rv, overall, gc, or default");
                System.exit(1);
            }
            ...
            while (f.hasMoreEvents()) {
                ...
                if (stack != null) {
                    ...
                    if (event.hasField("sampledThread")) {
                        if (event.getThread("sampledThread").getOSName().contains("main")) {
                            isMainThread = true;
                        }
                    }
...
//                    if (isMainThread) {
                    // System.out.println("[" + event.getThread("sampledThread").getOSName() + "]" + methods); 
