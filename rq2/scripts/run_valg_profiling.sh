...
CONTAINER_PATHS="sandbox-valg-profiling"

SCRIPT_PATH="sandbox-valg-profiling/home/Valg/tracemop/scripts"
OUTPUT_DIR="$SCRIPT_PATH/outputs-javamop"
...
for entry in "${projects[@]}"; do
    ...
    export PATH=$(pwd)/sandbox-valg-profiling/home/Valg/apache-maven/bin:$(pwd)/sandbox-valg-profiling/usr/lib/jvm/java-8-openjdk/bin:$(pwd)/sandbox-valg-profiling/home/Valg/aspectj-1.9.7/bin:$PATH
    export CLASSPATH=$(pwd)/sandbox-valg-profiling/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:$(pwd)/sandbox-valg-profiling/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:$(pwd)/sandbox-valg-profiling/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar
    # export SERIES_PATH="$SCRIPT_PATH/time-series/${project_name}_series"

    output_dir="output-$project_name"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash not_collect_traces.sh $repo $commit $output_dir"
    # $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH SERIES_PATH=$SERIES_PATH bash collect_traces-valgt-optimal.sh $repo $commit $output_dir"
