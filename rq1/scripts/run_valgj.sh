...
CONTAINER_PATHS="sandbox-valgj"

SCRIPT_PATH="sandbox-valgj/home/Valg/tracemop/scripts"
RESULTS_DIR="results-valgj"
OUTPUT_DIR="outputs-valgj"
...
for entry in "${projects[@]}"; do
    ...
    export PATH=$PATH:$(pwd)/sandbox-valgj/home/Valg/apache-maven/bin:$(pwd)/sandbox-valgj/usr/lib/jvm/java-8-openjdk/bin
    export CLASSPATH=$(pwd)/sandbox-valgj/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:$(pwd)/sandbox-valgj/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:$(pwd)/sandbox-valgj/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar

    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash install.sh false false false -valg"
    output_dir="output-$project_name"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash not_collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
    tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_PATH" "$output_dir"
    rm -rf "$SCRIPT_PATH/$output_dir"

    line=$(grep "^$project_name," online-hyperparameters.csv)
    alpha=$(echo "$line" | grep -oP 'alpha = \K[0-9.]+')
    epsilon=$(echo "$line" | grep -oP 'epsilon = \K[0-9.]+')
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash install.sh false false false -valg \"{${alpha},${epsilon},0.0001,5.0,0.0}\""
    ...
    tuned_json="$OFFTUNE_DIR/output-$project_name/project/tuned_hyperparameter.json"
    if [[ -f "$tuned_json" ]]; then
        spec_args=()
        for spec in $(jq -r 'keys[]' "$tuned_json"); do
            vals=$(jq -r ".\"$spec\" | [.alpha,.epsilon,.threshold,.Qc_init,.Qn_init] | @csv" "$tuned_json")
            vals="${vals//\"/}"  # remove quotes added by @csv
            spec_args+=(-spec "$spec" "\"{$vals}\"")
        done

        cmd="cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash install.sh false false false -valg ${spec_args[@]}"
        echo "Full command: $cmd"

        $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "$cmd"
    fi
    ...
    output_dir="output-$project_name"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash run_tests.sh $repo $commit $output_dir" &>> "$results_file"
    rm -rf "$SCRIPT_PATH/$output_dir"
done
