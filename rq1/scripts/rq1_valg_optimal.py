...
RESULTS_DIR = "results-valg-optimal"
OUTPUTS_DIR = "outputs-valg-optimal"
CSV_OUT = "rq1_valg_optimal.csv"
                    ...
                    if f is not None:
                        ...
                        for line in lines[1:]:
                            ...
                            if len(parts) >= 2:
                                count = int(parts[1])
                                total_unique += 1
                                total_redundant += max(0, count - 1)
    except Exception:
        pass
    return total_unique, total_redundant
...
for entry in PROJECTS:
    ...
    valgj_time = durations[0] if len(durations) > 0 else ""
    valgj_opt_time = durations[1] if len(durations) > 1 else ""
    valgt_time = durations[2] if len(durations) > 2 else ""
    valgt_opt_time = durations[3] if len(durations) > 3 else ""

    valgj_tar = os.path.join(OUTPUTS_DIR, f"output-{project}-valgj.tar.gz")
    valgj_opt_tar = os.path.join(OUTPUTS_DIR, f"output-{project}-valgj-optimal.tar.gz")
    valgt_tar = os.path.join(OUTPUTS_DIR, f"output-{project}-valgt.tar.gz")
    valgt_opt_tar = os.path.join(OUTPUTS_DIR, f"output-{project}-valgt-optimal.tar.gz")

    valgj_v = count_violations_from_tar(valgj_tar)
    valgj_opt_v = count_violations_from_tar(valgj_opt_tar)
    valgt_ut, valgt_rt = count_unique_traces(valgt_tar)
    valgt_opt_ut, valgt_opt_rt = count_unique_traces(valgt_opt_tar)

    rows.append([
        project,
        valgj_time, valgj_v,
        valgj_opt_time, valgj_opt_v,
        valgt_time, valgt_rt, valgt_ut,
        valgt_opt_time, valgt_opt_rt, valgt_opt_ut
    ])
...
with open(CSV_OUT, "w", newline="") as f:
    ...
    writer.writerow([
        ...
        "time", "violation",
        "time", "violation",
        "time", "redundant_traces", "unique_traces",
        "time", "redundant_traces", "unique_traces"
    ])
