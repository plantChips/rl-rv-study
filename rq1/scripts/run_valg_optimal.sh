...
CONTAINER_PATHS="sandbox-valg-optimal"

SCRIPT_PATH="sandbox-valg-optimal/home/Valg/tracemop/scripts"
RESULTS_DIR="results-valg-optimal"
OUTPUT_DIR="outputs-valg-optimal"
...
for entry in "${projects[@]}"; do
    ...
    export PATH=$(pwd)/sandbox-valg-optimal/home/Valg/apache-maven/bin:$(pwd)/sandbox-valg-optimal/usr/lib/jvm/java-8-openjdk/bin:$(pwd)/sandbox-valg-optimal/home/Valg/aspectj-1.9.7/bin:$PATH
    export CLASSPATH=$(pwd)/sandbox-valg-optimal/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:$(pwd)/sandbox-valg-optimal/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:$(pwd)/sandbox-valg-optimal/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar

    cp "$SCRIPT_PATH/valgj.jar" "$SCRIPT_PATH/no-track-no-stats-agent.jar"
    output_dir="output-${project_name}-valgj"
    ...
    export SERIES_PATH="$SCRIPT_PATH/time-series/${project_name}_series"
    cp "$SCRIPT_PATH/valgj-optimal.jar" "$SCRIPT_PATH/no-track-no-stats-agent.jar"
    output_dir="output-${project_name}-valgj-optimal"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH SERIES_PATH=$SERIES_PATH bash not_collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
    tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_PATH" "$output_dir"
    rm -rf "$SCRIPT_PATH/$output_dir"
    unset SERIES_PATH

    cp "$SCRIPT_PATH/valgt.jar" "$SCRIPT_PATH/track-no-stats-agent.jar"
    output_dir="output-${project_name}-valgt"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH bash collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
    tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_PATH" "$output_dir"
    rm -rf "$SCRIPT_PATH/$output_dir"

    export SERIES_PATH="$SCRIPT_PATH/time-series/${project_name}_series"
    cp "$SCRIPT_PATH/valgt-optimal.jar" "$SCRIPT_PATH/track-no-stats-agent.jar"
    output_dir="output-${project_name}-valgt-optimal"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && env PATH=$PATH CLASSPATH=$CLASSPATH SERIES_PATH=$SERIES_PATH bash collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
    tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_PATH" "$output_dir"
    rm -rf "$SCRIPT_PATH/$output_dir"
    unset SERIES_PATH
done
