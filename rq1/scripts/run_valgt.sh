...
CONTAINER_PATHS="sandbox-valgt"

SCRIPT_PATH="sandbox-valgt/home/Valg/tracemop/scripts"
RESULTS_DIR="results-valgt"
OUTPUT_DIR="outputs-valgt"
...
ONLINE_HYPERPARAMS="online-hyperparameters.csv"
OFFTUNE_DIR="off-tuned-hp"
...
for entry in "${projects[@]}"; do
    ...
    results_file="$RESULTS_DIR/$project_name.txt"

    export PATH=$(pwd)/sandbox-valgt/home/Valg/apache-maven/bin:$(pwd)/sandbox-valgt/usr/lib/jvm/java-8-openjdk/bin:$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/bin:$PATH
    export CLASSPATH=$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar

    output_dir="output-$project_name"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && bash install.sh true false false -valg && bash collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
    tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_Pexport CLASSPATH=$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:$(pwd)/sandbox-valgt/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jarATH" "$output_dir"
    rm -rf "$SCRIPT_PATH/$output_dir"

    alpha=0
    epsilon=0
    while IFS=, read -r proj line; do
        if [[ "$proj" == "\"$project_name\"" || "$proj" == "$project_name" ]]; then
            alpha=$(echo "$line" | grep -Po 'alpha\s*=\s*\K[0-9.]+')
            epsilon=$(echo "$line" | grep -Po 'epsilon\s*=\s*\K[0-9.]+')
            break
        fi
    done < "$ONLINE_HYPERPARAMS"

    output_dir="output-$project_name-online"
    $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && bash install.sh true false false -valg \"{$alpha,$epsilon,0.0001,5.0,0.0}\" && bash collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
    tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_PATH" "$output_dir"
    rm -rf "$SCRIPT_PATH/$output_dir"

    json_file="$OFFTUNE_DIR/output-$project_name/project/tuned_hyperparameter.json"
    if [ -f "$json_file" ]; then
        spec_args=""
        specs=$(grep -Po '"\K[^"]+(?=":\s*\{)' "$json_file")
        for spec in $specs; do
            block=$(awk -v s="$spec" 'BEGIN{RS="}";FS="\n"} $0 ~ s {print $0}' "$json_file")
            alpha=$(echo "$block" | grep -Po '"alpha":\s*\K[0-9.e-]+')
            epsilon=$(echo "$block" | grep -Po '"epsilon":\s*\K[0-9.e-]+')
            threshold=$(echo "$block" | grep -Po '"threshold":\s*\K[0-9.e-]+')
            Qc_init=$(echo "$block" | grep -Po '"Qc_init":\s*\K[0-9.e-]+')
            Qn_init=$(echo "$block" | grep -Po '"Qn_init":\s*\K[0-9.e-]+')

            spec_args="$spec_args -spec $spec \"{$alpha,$epsilon,$threshold,$Qc_init,$Qn_init}\""
        done

        output_dir="output-$project_name-offline"
        $APPTAINER_CMD "$CONTAINER_PATHS" bash -c "cd $SCRIPT_PATH && bash install.sh true false false -valg $spec_args && bash collect_traces.sh $repo $commit $output_dir" &>> "$results_file"
        tar -czf "$OUTPUT_DIR/$output_dir.tar.gz" -C "$SCRIPT_PATH" "$output_dir"
        rm -rf "$SCRIPT_PATH/$output_dir"
    else
        echo "[OFFLINE] No tuned hyperparameter file for $project_name" >> "$results_file"
    fi
done
