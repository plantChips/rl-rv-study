...
OUTPUTS_DIR = "outputs-valgj"
CSV_OUT = "rq1_valgj.csv"

DURATION_RE = re.compile(r"Duration:\s*(\d+)\s*ms")
FLOAT_RE = re.compile(r"^\s*(\d+(?:\.\d+)?)\s*$")

def extract_test_time(lines):
    for line in lines:
        if "Test time (s):" in line:
            m = re.search(r"Test time \(s\):\s*([0-9.]+)", line)
            if m:
                return m.group(1)
...
for entry in PROJECTS:
    ...
    # test_time = extract_test_time(lines)
    durations = DURATION_RE.findall("".join(lines))

    valg_time = durations[0] if len(durations) > 0 else ""
    valg_online_time = durations[1] if len(durations) > 1 else ""
    valg_offline_time = durations[2] if len(durations) > 2 else ""
    test_time = durations[3] if len(durations) > 3 else ""
    ...
    valg_v = count_violations_from_tar(base_tar)
    online_v = count_violations_from_tar(online_tar)
    offline_v = count_violations_from_tar(offline_tar)

    rows.append([
        test_time,
        valg_time, valg_v,
        valg_online_time, online_v,
        valg_offline_time, offline_v
    ])
...
with open(CSV_OUT, "w", newline="") as f:
    ...
    writer.writerow([
        "test_time",
        "valg_time", "valg_violation",
        "valg_online_time", "valg_online_violation",
        "valg_offline_time", "valg_offline_violation"
    ])
