...
BASE_DIR = Path("outputs-valgt")
OUT_CSV = "rq1_valgt_redundant.csv"
...
def sum_redundant_from_tar(tar_path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for utxt in Path(tmpdir).rglob("all-traces*/unique-traces.txt"):
            with utxt.open() as f:
                lines = f.readlines()
            for line in lines[1:]:
...
projects = [p.split("/")[1].split(",")[0] for p in PROJECTS]

with open(OUT_CSV, "w", newline="", buffering=1) as out:
    ...
    for project in projects:
        default_tar = BASE_DIR / f"output-{project}.tar.gz"
        online_tar = BASE_DIR / f"output-{project}-online.tar.gz"
        offline_tar = BASE_DIR / f"output-{project}-offline.tar.gz"

        default_sum = sum_redundant_from_tar(default_tar)
        online_sum = sum_redundant_from_tar(online_tar)
        offline_sum = sum_redundant_from_tar(offline_tar)
